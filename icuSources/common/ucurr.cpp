...
#include "ulocimp.h"
#if APPLE_ICU_CHANGES
// rdar://86727185 #282 (Please adopt InstallAPI & enable Deadstripping for: /usr/lib/libicucore.A.dylib)
#include "ucurrimp.h"
#endif // APPLE_ICU_CHANGES
...
#if APPLE_ICU_CHANGES
// rdar://79879611 #254 (SEED: Wrong currency symbol for en_KH)
    UChar32 s0 = 0;
#endif // APPLE_ICU_CHANGES
    ec2 = U_ZERO_ERROR;
#if APPLE_ICU_CHANGES
// rdar://62548298 #197 ([Mixed locale] Improve locale fallback for currency symbols)
// rdar://77347002 #222 (iOS: Sky: English/SA: Currency placement is incorrect in system and settings example.)
// rdar://97268297&97100652 #337 (Currency formatting with locale “zh-Hant_US@currency=USD” returns “US$” rather than “$”)
    UBool didFallBackByCountry = false;
    LocalUResourceBundlePointer languageRB(ures_open(U_ICUDATA_CURR, loc.data(), &ec2));
    LocalUResourceBundlePointer countryRB(ures_openWithCountryFallback(U_ICUDATA_CURR, loc.data(), &didFallBackByCountry, &ec2));
    UScriptCode languageLocaleScripts[5];
    if (didFallBackByCountry) {
        uscript_getCode(loc.data(), languageLocaleScripts, 5, &ec2);
        if (U_FAILURE(ec2)) {
...
// rdar://97268297&97100652 #337 (Currency formatting with locale “zh-Hant_US@currency=USD” returns “US$” rather than “$”)
        ...
        if (s == nullptr || u_strcmp(s, currency) == 0) {
            if (U_FAILURE(ec2)) {
                ec2 = U_ZERO_ERROR;
            }
...
(Apple ICU updates link to be https://www.iso.org/iso-4217-currency-codes.html)
*/
