...
//
//------------------------------------------------------------------------------
RBBIRuleScanner57::RBBIRuleScanner57(RBBIRuleBuilder57 *rb)
{
    fRB                 = rb;
    fScanIndex          = 0;
    fNextIndex          = 0;
    ...
    fSymbolTable = new RBBISymbolTable57(this, rb->fRules, *rb->fStatus);
...
//
//------------------------------------------------------------------------------
RBBIRuleScanner57::~RBBIRuleScanner57() {
    delete fSymbolTable;
    if (fSetTable != NULL) {
...
//
//------------------------------------------------------------------------------
UBool RBBIRuleScanner57::doParseActions(int32_t action)
{
    RBBINode *n       = NULL;
...
//
//------------------------------------------------------------------------------
void RBBIRuleScanner57::error(UErrorCode e) {
...
//
//------------------------------------------------------------------------------
void RBBIRuleScanner57::fixOpStack(RBBINode::OpPrecedence p) {
    RBBINode *n;
    // printNodeStack("entering fixOpStack()");
    for (;;) {
        n = fNodeStack[fNodeStackPtr-1];   // an operator node
        if (n->fPrecedence == 0) {
            RBBIDebugPuts("RBBIRuleScanner57::fixOpStack, bad operator node");
            error(U_BRK_INTERNAL_ERROR);
            return;
        }
...
//
//------------------------------------------------------------------------------
void RBBIRuleScanner57::findSetFor(const UnicodeString &s, RBBINode *node, UnicodeSet *setToAdopt) {
...
//
//------------------------------------------------------------------------------
UnicodeString RBBIRuleScanner57::stripRules(const UnicodeString &rules) {
...
//
//------------------------------------------------------------------------------
UChar32  RBBIRuleScanner57::nextCharLL() {
    UChar32  ch;
...
//
//------------------------------------------------------------------------------
void RBBIRuleScanner57::nextChar(RBBIRuleChar &c) {
    ...
    for (;;) {
        ...
        if (tableEl->fPushState != 0) {
            ...
            if (fStackPtr >= kStackSize) {
                error(U_BRK_INTERNAL_ERROR);
                RBBIDebugPuts("RBBIRuleScanner57::parse() - state stack overflow.");
                fStackPtr--;
            }
        ...
        if (tableEl->fNextState != 255) {
            ...
            if (fStackPtr < 0) {
                error(U_BRK_INTERNAL_ERROR);
                RBBIDebugPuts("RBBIRuleScanner57::parse() - state stack underflow.");
                fStackPtr++;
            }
        }
...
//------------------------------------------------------------------------------
#ifdef RBBI_DEBUG
void RBBIRuleScanner57::printNodeStack(const char *title) {
    int i;
...
//
//------------------------------------------------------------------------------
RBBINode  *RBBIRuleScanner57::pushNewNode(RBBINode::NodeType  t) {
    if (U_FAILURE(*fRB->fStatus)) {
        return NULL;
    }
    fNodeStackPtr++;
    if (fNodeStackPtr >= kStackSize) {
        error(U_BRK_INTERNAL_ERROR);
        RBBIDebugPuts("RBBIRuleScanner57::pushNewNode - stack overflow.");
...
//
//------------------------------------------------------------------------------
void RBBIRuleScanner57::scanSet() {
    UnicodeSet    *uset;
    ParsePosition  pos;
    int            startPos;
    int            i;
