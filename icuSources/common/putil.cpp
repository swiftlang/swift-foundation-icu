...
#if U_PLATFORM_IS_DARWIN_BASED && __has_include(<os/eligibility.h>)
#include <os/eligibility.h>
#endif // U_PLATFORM_IS_DARWIN_BASED && __has_include(<os/eligibility.h>)
        
U_CAPI bool U_EXPORT2
uaprv_onCalciumDevice() {
#if U_PLATFORM_IS_DARWIN_BASED && __has_include(<os/eligibility.h>)
#if !TARGET_OS_OSX
    os_eligibility_answer_t eligibilityAnswer;
    int eligibilityError = os_eligibility_get_domain_answer(OS_ELIGIBILITY_DOMAIN_CALCIUM, &eligibilityAnswer, nullptr, nullptr, nullptr);

    // NOTE: For ICU's purposes, we don't distinguish between return values from os_eligibility:
    // OS_ELIGIBILITY_ANSWER_INVALID, OS_ELIGIBILIY_ANSWER_NOT_YET_AVAILABLE, and a nonzero return code
    // are all considered equivalent to OS_ELIGIBILITY_ANSWER_NOT_ELIGIBLE (i.e., all mean "we're not
    // running on a China-SKU device").  If you're getting non-China-SKU results on a China-SKU device,
    // you can ssh into the device and use `eligibility_util getAnswer OS_ELIGIBILITY_DOMAIN_CALCIUM`
    // to find out what we're really getting back from os_eligibility.
    // NOTE: This function assumes the call to os_eligibility is cheap; we can cache the result here
    // if the call turns out to be too expensive.
    return eligibilityError == 0 && eligibilityAnswer == OS_ELIGIBILITY_ANSWER_ELIGIBLE;
#else
    return false;
#endif // !TARGET_OS_OSX
#else
