...
// rdar://64921192 #122 ef6647ddca.. 48 Processes had 191 icu::UnicodeString Objects in them which total’d ~573K across the system
    gMetaZoneIDTable = uhash_open(uhash_hashUChars, uhash_compareUChars, nullptr, &status);
#else
...
#if APPLE_ICU_CHANGES
// rdar://64921192 #122 ef6647ddca.. 48 Processes had 191 icu::UnicodeString Objects in them which total’d ~573K across the system
// rdar://97937093 #nnn (sub-TLF: Integrate ICU 72)
        if (uhash_get(gMetaZoneIDTable, uMzID.getAlias()) == NULL) {
            uhash_put(gMetaZoneIDTable, uMzID.getAlias(), uMzID.getAlias(), &status);
...
// rdar://64921192 #122 ef6647ddca.. 48 Processes had 191 icu::UnicodeString Objects in them which total’d ~573K across the system
    ...
    return (const char16_t*)uhash_get(gMetaZoneIDTable, localMzid.getTerminatedBuffer());
#else
