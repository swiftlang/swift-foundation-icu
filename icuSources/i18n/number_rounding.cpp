...
Precision Precision::constructIncrementSignificant(double increment, int32_t minSig, int32_t maxSig) { // Apple rdar://52538227
    ...
    return {RND_INCREMENT_SIGNIFICANT, union_};
}
...
// rdar://97937093 #nn sub-TLF: Integrate ICU 72
        case Precision::RND_INCREMENT_SIGNIFICANT:
            // First round to increment (after converting it to uint64_t + magnitude)
            {
                DecimalQuantity dq;
                dq.setToDouble(fPrecision.fUnion.incrSig.fIncrement);
                dq.roundToInfinity();
                int32_t magnitude = dq.adjustToZeroScale();
                value.roundToIncrement(
                        dq.toLong(), magnitude,
