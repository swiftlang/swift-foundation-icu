    ...
    if (fStamp[UCAL_DAY_OF_WEEK] >= kMinimumUserStamp &&
        ...
        fStamp[UCAL_DAY_OF_WEEK_IN_MONTH] == kInternallySet) {
            ...
            if ( monthStampDelta >= 1 && monthStampDelta <= 3 && dateStampDelta >= 1 && dateStampDelta <= 3 ) {
            	// TODO: In ICU 74, this function was changed to "const", which breaks the code
            	// below.  For the time being, we're working around this by casting away the const,
            	// but we really need to find a better solution.	--rtg 10/25/23
            	Calendar* nonConstThis = const_cast<Calendar*>(this);
                ...
                nonConstThis->fStamp[UCAL_WEEK_OF_YEAR] = kUnset;
                nonConstThis->fStamp[UCAL_WEEK_OF_MONTH] = kUnset;
                nonConstThis->fStamp[UCAL_DAY_OF_WEEK_IN_MONTH] = kUnset;
...
// rdar://14271954 518a7e70de.. When resolving fields, explicit day + month should outweigh later explicit weekday + implicit fields
    if (restoreWeekOfInternalSet) {
        ...
        nonConstThis->fStamp[UCAL_WEEK_OF_YEAR] = kInternallySet;
        nonConstThis->fStamp[UCAL_WEEK_OF_MONTH] = kInternallySet;
        nonConstThis->fStamp[UCAL_DAY_OF_WEEK_IN_MONTH] = kInternallySet;
    }
#endif  // APPLE_ICU_CHANGES
