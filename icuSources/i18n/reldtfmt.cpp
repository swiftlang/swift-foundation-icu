...
// rdar://114975916 ([Finder]: US: 23B40a: Inconsistent formatting in timestamps in list view)
    UResourceBundle *rb = ures_open(nullptr, fLocale.getBaseName(), &status);
    bool hasAtTimeResource = true;
    ...
    if (U_FAILURE(status)) {
        status = U_ZERO_ERROR;
        dateTimePatterns.adoptInstead(ures_getByKeyWithFallback(rb, "calendar/gregorian/DateTimePatterns", (UResourceBundle*)nullptr, &status));
        hasAtTimeResource = false;
    }
    
    if(U_SUCCESS(status)) {
        int32_t patternsSize = ures_getSize(dateTimePatterns.getAlias());
        if (hasAtTimeResource || patternsSize > kDateTime) {
            int32_t resStrLen = 0;
            int32_t glueIndex = 0;
            if (hasAtTimeResource) {
