 ...
 * uloc_canonicalize). This is a C wrapper for C++ Locale::createCanonical.
 * This is currently Apple-specific but in uloc.h instead of ualoc.h
 * because this capability might be needed on Safari for Windows.
 * At some point this capability will be in open-source ICU, see
 * https://unicode-org.atlassian.net/browse/ICU-21506
 *
 * @param localeID the locale ID to convert to canonical form
 * @param name the buffer for the canonical ID result
 * @param nameCapacity the size of the name buffer
 * @param err error information if retrieving the full name failed
 * @return the actual buffer size needed for the full name.  If it's greater
 * than nameCapacity, the returned full name will be truncated.
 * @internal
