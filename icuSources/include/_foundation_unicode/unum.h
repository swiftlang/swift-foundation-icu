...
// rdar://
#ifndef U_HIDE_INTERNAL_API
  /** Apple addition for rdar://39240173.
   * In open-source ICU 60 and earlier, unum_formatDouble pinned
   * double to string conversion at DBL_DIG=15 (from <float.h>)
   * significant digits; beginning in ICU 61, several extra digit
   * digits could be produced. However, by default Apple ICU
   * still pins to 15 digits if UNUM_SIGNIFICANT_DIGITS_USED is
   * false and UNUM_MAX_FRACTION_DIGITS > 15; this is to improve
   * compatibility with Numbers. This default can be overriden
   * by setting UNUM_FORMAT_WITH_FULL_PRECISION to true.
   * @internal */
